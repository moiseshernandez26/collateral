...
export const PONG = {
  ...
  // Slower than a Pong normally wants to be, on purpose. The limiting factor
  // here is not playability, it's whether a model round-trip fits inside the
  // ball's flight: the slow-motion window buys most of that, and a calmer base
  // speed buys the rest — the segments either side of it, where the agent has
  // already committed and the ball is travelling at full speed.
  baseSpeed: 180,
  maxSpeed: 380,
  speedStep: 10, // added to the ball's speed on every paddle hit
  ...
  readTimeoutMs: 8000,
  // Fraction of the court the ball must cross before the agent is woken. Woken
  // early on purpose: the agent gets the whole run-up to its own paddle to
  // answer in, and most of that run-up is spent in slow motion.
  approachAt: 0.72,
...
// A duel round doesn't serve the moment Pong opens: the ball waits until the
// agent checks in with pong_ready, because starting a live match under its feet
// is how the first points went by before it knew it had a paddle.
